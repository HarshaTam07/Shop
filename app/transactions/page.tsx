                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-right">
                      <p className="text-lg font-bold text-green-600 dark:text-green-400">
                        ₹{(transaction.finalTotal + (transaction.customAmount || 0)).toFixed(2)}
                      </p>
                      {transaction.customAmount && transaction.customAmount > 0 && (
                        <p className="text-xs text-purple-600 dark:text-purple-400">
                          (Cart: ₹{transaction.finalTotal.toFixed(2)} + Custom: ₹{transaction.customAmount.toFixed(2)})
                        </p>
                      )}
                      {transaction.paidAmount !== undefined && transaction.paidAmount < (transaction.finalTotal + (transaction.customAmount || 0)) && (
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-300 dark:border-gray-600">
                    ...
                    {transaction.paidAmount !== undefined && (
                      <>
                        ...
                        {transaction.paidAmount < (transaction.finalTotal + (transaction.customAmount || 0)) && (
                          <div className="flex justify-between text-xs text-orange-600 dark:text-orange-400 mt-1 font-semibold">
                            <span>Balance Due:</span>
                            <span>₹{((transaction.finalTotal + (transaction.customAmount || 0)) - transaction.paidAmount).toFixed(2)}</span>
                          </div>
                        )}
                      </>
                    )}
